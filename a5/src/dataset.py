...
class NameDataset(Dataset):
    ...
    def __getitem__(self, idx):
        try:
            ...
            # reaches eof
            print(self.data[idx], flush=True)
            blank = self.MASK_CHAR*self.block_size
            return torch.tensor([self.stoi[c] for c in blank], dtype=torch.long), \
                torch.tensor([self.stoi[c] for c in blank], dtype=torch.long)
...
class CharCorruptionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if len(doc) < 4:
            ...
            x = torch.tensor([self.stoi[ch] for ch in masked_string], dtype=torch.long)
            y = torch.tensor([self.stoi[ch] for ch in masked_string], dtype=torch.long)
            return (x, y)
        # Important: when truncating, keep the prefix since the peoples' names (which are the answers to finetune task questions) are there.
        # Content sometimes mentions their names again, but it's much harder. 
        # Removing words from prefix hurts performance a lot (9% with causal attention, 12 layers, 12 heads, 768d embedding, 
        # down from 23% with causal attention and just 4 layers, 8 heads, 256d embedding)
        truncated_len = random.randint(4, min(len(doc), int(self.block_size*7/8)))
        doc = doc[:truncated_len]
        masked_len = random.randint(int(truncated_len*3/16), int(truncated_len*5/16))
        mask_start = random.randint(0, truncated_len-masked_len)
        prefix = doc[:mask_start]
        mask = doc[mask_start:mask_start+masked_len]
        suffix = doc[mask_start+masked_len:]
        ...
        x = torch.tensor([self.stoi[ch] for ch in input], dtype=torch.long)
        y = torch.tensor([self.stoi[ch] for ch in output], dtype=torch.long)
        return (x, y)
