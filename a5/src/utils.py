...
def sample(model, x, steps, temperature=1.0, sample=False, top_k=None):
    ...
    for k in range(steps):
        ...
        if model.synthesizer:
            # pad to block size if synthesizer since some synthesizer parameter dimensions are block size dependent
            x_cond = F.pad(x_cond, (0, model.block_size-x.size(1)), 'constant', 0)
        logits, _ = model(x_cond)
        # pluck the logits at the final step and scale by temperature
        logits = logits[:, x.size(1)-1, :] / temperature
