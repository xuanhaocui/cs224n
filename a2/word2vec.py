...
def naiveSoftmaxLossAndGradient(
    ...
    ### to integer overflow. 
    y_hat = np.exp(outsideVectors.dot(centerWordVec))/np.sum(np.exp(outsideVectors.dot(centerWordVec)))
...
def negSamplingLossAndGradient(
    ...
    U_T = outsideVectors[indices] # (num words sampled + 1, word vector length)
    U_T[1:] *= -1
    sigs = sigmoid(U_T.dot(centerWordVec))
    one_minus_sigs = 1-sigs
    loss = -np.sum(np.log(sigs))
    # gradCenterVec = -2*one_minus_sigs[0]*outsideVectors[outsideWordIdx].T+U_T.T.dot(sigs)
    gradCenterVec = -U_T.T.dot(one_minus_sigs)
    gradOutsideVecs = np.zeros_like(outsideVectors)
    gradOutsideVecs[outsideWordIdx] = -one_minus_sigs[0]*centerWordVec
    for i,idx in enumerate(negSampleWordIndices):
        gradOutsideVecs[idx] += one_minus_sigs[i+1]*centerWordVec
    ### END YOUR CODE
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    for outside in outsideWords:
        outsideIdx = word2Ind[outside]
        cur_loss, gradCenter, gradOutside = word2vecLossAndGradient(centerWordVectors[centerIdx], outsideIdx, outsideVectors, dataset)
