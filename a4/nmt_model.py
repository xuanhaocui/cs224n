...
class NMT(nn.Module):
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        enc_hiddens, (last_hidden, last_cell) = self.encoder(nn.utils.rnn.pack_padded_sequence(X, torch.tensor(source_lengths)))
        enc_hiddens = nn.utils.rnn.pad_packed_sequence(enc_hiddens, batch_first=True)[0]
        hidden_cat = torch.squeeze(torch.cat(torch.split(last_hidden, 1), dim=-1), dim=0)
        init_decoder_hidden = self.h_projection(hidden_cat)
        cell_cat = torch.squeeze(torch.cat((torch.split(last_cell, 1)), dim=-1), dim=0)
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        for Y_t in torch.split(Y, 1):
            ...
            Ybar_t = torch.concat((Y_t, o_prev), dim=-1) # (b, e+h)
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        a_t = torch.squeeze(torch.bmm(torch.unsqueeze(alpha_t, dim=-2), enc_hiddens), dim=-2)
