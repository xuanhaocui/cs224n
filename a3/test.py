...
from utils.parser_utils_cuda import minibatches, load_test_data, AverageMeter
...
if __name__ == "__main__":
    ...
    print("INITIALIZING")
    print(80 * "=")
    parser, embeddings, test_data = load_test_data(True)

    start = time.time()
    device = "cuda" if torch.cuda.is_available() else "cpu"
    model = ParserModel(embeddings, hidden_size=500).to(device)
    parser.model = model
    print("took {:.2f} seconds\n".format(time.time() - start))

    input_dir = "results/{}/".format(args.folder)
    input_model_path = input_dir + "model.weights"
    input_embeddings_path = input_dir + "embeddings.npy"
    
    print(80 * "=")
    print("TESTING")
    print(80 * "=")
    print("Restoring the best model weights found on the dev set")
    parser.model.load_state_dict(torch.load(input_model_path))
    parser.model.embeddings = torch.tensor(np.load(input_embeddings_path))
    parser.model.print()
    print(test_data)
